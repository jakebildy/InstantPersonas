...
function checkSkimmability(headline: string, topic: string): boolean {
  ...
  const skimmable =
    firstThree.some((word) => topic.includes(word)) ||
    lastThree.some((word) => topic.includes(word));
  ...
  if (req.body) {
    ...
    try {
      ...
      const pageRanks = await axios.post(
        ...
        [
          {
            targets: serpData.data.tasks[0].result[0].items
              .slice(6, 10)
      ...
      const DIFFICULTY_SCORE = calculateDifficultyScore(
        pageRanks.data.tasks[0].result[0].items,
      );

      // split the headline up into each word and compare the lower case word with the list of power words
      const POWER_WORDS_INCLUDED = headline
        .split(" ")
        .map((word: string) => word.toLowerCase())
        .filter((word: string) =>
          POWER_WORDS.map((pw) => pw.toLowerCase()).includes(word),
        );
