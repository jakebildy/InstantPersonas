...
const STRIPE_ENDPOINT_SECRET: string | undefined = process.env.STRIPE_ENDPOINT_SECRET;
...
async function fulfillOrder(session: Session): Promise<void> {
    ...
    user = await User.updateOne({ stytchID: stytchID }, { stripeCustomerId, stripeSubscriptionId });
